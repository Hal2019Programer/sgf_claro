...
<html>
	...
	<body>
		<div id="container1">
			<!-- <div id="main-col3" style="background-image: url('../imagenes/BoletaVentaCetic_8.jpg');"> -->
			<div id="main-col3">
				<?php
				...
				$sql= mysqli_query ($Conexion,$sql_regventas) or die ("Error al traer los datos de regventas para impresion de la factura.");
				...
				<form name="usuario" action="" method="post">
					...
					<div style="font-size:23px"><br></div> <!-- Espacio de separación entre linea -->
					<div>
						<span id="prn1" style="width:900px;"></span> <!-- Espacio inicial -->
						<span id="prn1" style="width:103px;"><?php echo substr("0000".(string)$ser,-4);?></span> <!-- Serie -->
						<span id="prn1" style="width:255px;"><?php echo substr("0000".(string)$ncp,-6);?></span> <!-- Numero de comprobante de pago -->
					</div>
					<div style="font-size:15px"><br></div> <!-- Espacio de separación entre linea -->
					<div>
						<span id="prn1" style="width:210px;"></span> <!-- Espacio inicial -->
						<span id="prn2" style="width:600px;"><?php echo valfield($Conexion,"clientes","nom_rzs_cli","id_cli",$idc);?></span><!-- Nombre del cliente -->
						<span id="prn1" style="width:110px;"></span> <!-- Espacio entre nombre del cliente y fecha -->
						<span id="prn2" style="width:300px;"><?php echo $fvt;?></span> <!-- Fecha de venta -->
					</div>
					<div style="font-size:11px"><br></div> <!-- Espacio de separación entre linea -->
					<div>
						<span id="prn1" style="width:210px;"></span> <!-- Espacio inicial -->
						<span id="prn2" style="width:600px;"><?php echo valfield($Conexion,"clientes","direcc_cli","id_cli",$idc), " - ", valfield($Conexion,"clientes","lugar_cli","id_cli",$idc);?></span><!-- Dirección y lugar del cliente -->
						<span id="prn1" style="width:110px;"></span> <!-- Espacio entre dirección de cliente y DNI/RUC -->
						<span id="prn2" style="width:200px;"><?php echo valfield($Conexion,"clientes","dni_ruc_cli","id_cli",$idc);?></span> <!-- DNI/RUC del cliente -->
					</div> 
					<div style="font-size:13px"><br></div> <!-- Espacio de separación entre linea -->
				...
				<table border='0' cellspacing='0' cellpadding='0'>
					...
					while($resul = mysqli_fetch_array($sql, MYSQLI_ASSOC))
					{
						...
						?>
						<tr align='left'>
							<td><span id="prn1" style="width:70px;">&nbsp;</span></td> <!-- Espacio inicial -->
							<td><span id="prn3" style="width:64px;"><?php echo 1;?></span></td> <!-- Cantidad -->
							<td><span id="prn2" style="width:835px;"><?php echo $cpr, " ", $abp, " ", $apl; if(!empty($spr)) echo "  Serie:",$spr; if(!empty($ime)) echo "  Imei:",$ime; if(!empty($icc)) echo "  ICC:",$icc; ?></span></td> <!-- Código de producto, Nombre de producto, Nombre del plan, Serie, Imei o ICC -->
							<td><span id="prn3" style="width:83px; text-align:right;"><?php echo number_format($imp/1.18, 2, '.', '');?></span></td> <!-- Importe P.Unit. -->
							<td><span id="prn3" style="width:125px; text-align:right;"><?php echo number_format($imp/1.18, 2, '.', '');?></span></td> <!-- Importe final -->
						</tr>
						<?php
					}
					while ($contador<6)
				...
				<div style="font-size:48px"><br></div> <!-- Espacio de separación entre linea -->
			  <?php if ($tdc=="Factura")
					{ ?>
						<div>
						...
						<div>
							<div style="font-size:28px"><br></div> <!-- Espacio de separación entre linea -->
							<span id="prn1" style="width:1136px;">&nbsp;</span> <!-- Espacio inicial -->
							<span id="prn3" style="width:125px; text-align:right;"><?php echo number_format($bipg, 2, '.', '');?></span><br><!-- SUB TOTAL= Base imponible a productos no gravados -->
							<div style="font-size:12px"><br></div> <!-- Espacio de separación entre linea -->
							<span id="prn1" style="width:1136px;">&nbsp;</span> <!-- Espacio inicial -->
							<span id="prn3" style="width:125px; text-align:right;"><?php echo number_format($igvv, 2, '.', '');?></span><br><!-- IGV -->
							<div style="font-size:12px"><br></div> <!-- Espacio de separación entre linea -->
							<span id="prn1" style="width:1136px;">&nbsp;</span> <!-- Espacio inicial -->
							<span id="prn3" style="width:125px; text-align:right;"><?php echo number_format($mntt, 2, '.', '');?></span><!-- Importe total en números -->
						</div>
			  <?php } ?>			
			</div><!--Fin de main-col-->
